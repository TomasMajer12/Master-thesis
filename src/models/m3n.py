...
def chain_edges(seq_len, device=None):
    """Build edge list for a chain graph: 0-1-2-..-(T-1).

    Returns:
        edges: LongTensor of shape [seq_len - 1, 2]
               Each row is (t, t+1).
    """
...
class M3N(nn.Module):
    ...
    Args:
        backbone:    nn.Module mapping [batch, num_nodes, input_dim]
    ...
    def __init__(self, backbone, num_classes):
        ...
        # Pairwise weight matrix — shared across all edges.
        # W[i, j] = affinity between class i at node u and class j at node v.
        # Initialized near zero so that early training is dominated by unary
        # potentials (the backbone), which is easier to learn from.
        self.pairwise = nn.Parameter(torch.randn(num_classes, num_classes) * 0.1)
