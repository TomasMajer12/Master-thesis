...
def viterbi_decode(unary, pairwise):
    ...
    dp = torch.zeros(T, batch, K, device=device, dtype=unary.dtype)
...
def loss_augmented_viterbi(unary, pairwise, y_true):
    ...
    # Build the Hamming loss term: 1/T for every class != y_true, 0 at true.
    # loss_term[b, t, k] = 1/T if k != y_true[b, t], else 0.0
    #
    # Normalised by T (Franc-Yermakov convention) so the total
    # Hamming task loss lives in [0, 1]. 
    loss_term = torch.ones(batch, T, K, device=unary.device, dtype=unary.dtype) / T
