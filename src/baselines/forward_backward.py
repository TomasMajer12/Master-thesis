...
Computes marginal posteriors P(y_t | x_1, ..., x_T) and predicts:
    y_t = argmax_y P(y_t = y | X)

This minimizes expected Hamming loss — it is the theoretically best possible
classifier when the true HMC parameters are known.

Note: this baseline requires symbolic (discrete) observations.
It cannot be applied directly to visual (image) inputs.
"""
