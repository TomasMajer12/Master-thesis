...
The training loss is the margin-rescaling form of
Section~\ref{subsec:max-margin}: for each training pair
$(x, y)$, the loss-augmented oracle returns the
most-violating labeling $y^{\mathrm{LA}}$, and the loss is
\begin{equation*}
    \Delta(x, y, w) \;=\; f(x, y^{\mathrm{LA}}) +
    \ell(y, y^{\mathrm{LA}}) - f(x, y),
\end{equation*}
clamped at zero for safety, though the inner $\max$ already
guarantees non-negativity. The oracle is exact via Viterbi on
chains. The corresponding implementation is the
...
The training loss is the LP-relaxed surrogate of
Section~\ref{subsec:lp-m3n}: each training example carries its
own auxiliary variables $\phi^{i}$, and the loss reads
\begin{equation*}
    \Delta^{\mathrm{LP}}(x^{i}, y^{i}, w, \phi^{i})
    \;=\; U(x^{i}, y^{i}, w, \phi^{i})
    - \langle w, \psi(x^{i}, y^{i}) \rangle.
\end{equation*}
No inference oracle is invoked at training time --- the $\max$
operations in $U$ are local and evaluated in closed form,
using the bank of $\phi^{i}$ tensors described in
Section~\ref{subsec:method-training-phi}. This is the training
path used on the Sudoku graph; it is also exercised on chains
in the tightness-validation experiment of
Chapter~\ref{chap:experiments}. The corresponding
implementation is the \texttt{lp\_m3n\_loss} module, selected
by \texttt{loss: lp\_m3n} together with
\texttt{inference.train: lp}.
...
backbone and the small ($K \times K$) pairwise matrix at low
training-set size $N$, discussed empirically in
