...
training-set size $N$ on symbolic Sudoku. Line: mean across
three seeds; shaded band: empirical min--max range across
seeds (used in place of mean $\pm$ std because the error scale
is so small that the std crosses the physical floor at zero).
The solver-oracle floor of $0$ is reached for every $N \geq 100$.}
